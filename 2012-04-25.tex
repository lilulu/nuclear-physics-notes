...
\lecture{Point Kinetics Without Feedback} \label{PKE-without-feedback}
In this chapter, I use `generalized PKE' to refer to the formulation using integrated cross sections terms, and `classical PKE' specifically cast integrated cross sections into $\rho, \beta, \Lambda$. 
...
\item  \textcolor{blue}{Delayed yields $\nu_d$ depend on fissioning species and neutron energy.}
  \begin{itemize}
  ...
  \item Delayed neutron fraction $\beta$: absolute yield (\# of delayed neutrons per fission) divided by $\bar{\nu}$ (total fission neutrons) = percentage of neutrons that are delayed.
  \item We talk about $\beta_{\mathrm{effective}}$ once it is averaged over all isotopes and flux. 
  \item $\beta$ changes as a function of time through breeding of Pu-239 in U-239 cycle, though the variations in BU are on a much larger time scale than usual range of application of PKEs so typically we ignore the change in $\beta$ for short transients. 
...
\clearpage
\topic{Generalized PKE from Diffusion Equation}
This lecture was given by Prof. Smith for 22.S904. Recall the \uline{steady-state diffusion equation}: 
\small
\begin{align}
...
Next we will start from the time-dependent neutron diffusion equation and go through various steps to arrive at the generalized PKE. 
\begin{enumerate}
\item The \uline{time-dependent neutron diffusion equation}: now delayed neutrons must be treated explicitly. $\beta^j = \Sum_i \beta_i^j$ is the delayed fission fraction (0.66\% for instance); we use $i$ is for isotropes, $j$ is for energy groups.
  \begin{align}
...
\notag \\
    ...
    \ppt C_i (\vecr, t) &= \Sum_j \beta_i^j \int_0^{\infty} \nu \Sigma_f^j (\vecr, E', t) \phi(\vecr, E', t) \dE'  - \lambda_i C_i (\vecr, t) 
  \end{align}
...
where $S(\vecr, E)$ is the Shape function,  $T(t)$ is the amplitude function (remember this approximation may not always be valid, and certainly in some cases it would not be a good approximation). Plug it into the above expressions. 
  \begin{align}
    ...
    &+ \Sum_i \chi_d^i (E) \lambda_i C_i (\vecr, t) + Q(\vecr, E, t)   \\
    \ppt C_i (\vecr, t) &= \left\{ \Sum_j \beta_i^j \int_0^{\infty} \nu \Sigma_f^j (\vecr, E', t) S(\vecr, E') \dE' \right\} T(t) - \lambda_i C_i (\vecr, t) 
  \end{align}
...
\item Assume \hi{prompt fission spectrum $\chi_p^j$ is independent of fissioning species $j$}, that is, we drop the $j$ dependency on prompt terms $\chi_p^j, \beta^j$, 
...
\item Assume \hi{delayed neutron yield $\beta$ is independent of space}, that is, $\beta (\vecr) \to \beta$. 
...
\topic{Multi-Group Generalized PKEs}
Based on Eq.~\ref{PKE-6} and Eq.~\ref{PKE-8}, we can further define multi-group PKEs. 
\begin{enumerate}
...
\item \hi{2-group PKEs}: assumptions:
  \begin{enumerate}
  ...
  Interpretaion: 
  \begin{enumerate}
  \item Notice we include the adjoint flux as weighting factors. Technically the parameters in the previous sections should include corresponding weighting factors as well. 
  \item $\beta$ has no $T(t)$ temporal dependency. $\beta$ only depends on $\phi_1^*$ but not $\phi_2^*$, because all delayed neutrons are born in group 1, hence we do not care about the neutron importance in group 2.  
  \item$\Lambda$ is the prompt neutron life time, thus only the importance of group 1 neutrons matters.
  \item Keep in mind that the classical PKE expression that we would show in the next subsection looks simple; it is how these parameters are defined that is involved. 
  \end{enumerate}



  If we drop the $\vecr, t$ dependency, we get a cleaner look of the 2-group PKE: 
    \small
  \begin{align}
...
\clearpage
\topic{Classical PKE}
For the classical PKE approach, we lump the integrals of cross sections and shape functions into simple parameters like $\rho, \beta$ and $\Lambda$. To start, for simplicity concern we omit the dependency on the following terms: $D(\vecr, E, t), \Sigma_t (\vecr, E, t), \Sigma_f(\vecr, E, t),  S(\vecr, E), \chi(E)$. 

\subtopic{Classical PKE Parameters}
\begin{enumerate}
...
\eqn{ \rho = \frac{\mbox{net production}}{\mbox{perturbation denomenator (if all neutrons show up instantaneously)}} }
\scriptsize
\begin{align}
...
A special word about reactivity: \textbf{$\rho(t)$ does not describe how much neutrons there are at a specific time; it describes how much neutrons there would be eventually.} 
\begin{itemize}
  ...
  \item Dynamic $\rho$ changes as a function of time and indicates changes from an initially stable system. 
    \eqn{ \rho(t) &= \frac{\rho_f - \rho_i}{\rho_f} }
\end{itemize}

The unit of $\rho$ can be confusing. See Table~\ref{common-rho-units}. 
\begin{table}[h]
  \centering
  ...
  \end{tabular}
  \caption{Common Reactivity Units}\label{common-rho-units}
\end{table}

\item The delayed neutron fraction (notice although this is called delayed neutron fraction, there is $\chi_p^j(E)$ term in it). If we ignore fissioning spectrum, $\beta \approx \Sum_i \beta_i$. 
\begin{align}
...
\begin{table}[h]
  ...
  \begin{tabular}{|c|c|c|c|} \hline
    & LWR & CANDU & Fast Reactors \\ \hline
    $\Lambda$ & $5\times 10^{-5}$ & $1 \times 10^{-3}$ & $1 \times 10^{-6}$ \\ \hline
    $\beta$ & 0.0075 & 0.06 & 0.0035 \\ \hline
    $\lambda$ & 0.1 & 0.1 & 0.1 \\ \hline
  \end{tabular}
  \caption{Typical Values for PKE Parameters} 
\end{table}



\clearpage
\subtopic{Classical PKE using Parameters}
...
\clearpage
\topic{Analytical Solution with One Precursor Group}
This lecture was presented by Prof. Forget on Sept. 20, 2013. We are interested in solving the classical PKEs with one effective delayed neutron precursor group. 

Problem statement: we hold steady state before $t=0$ such that: $\rho(0) = 0, P(0) = P_0, C(0) = \frac{\beta}{\lambda \Lambda} P_0$. Then we perturbe the reactivity at $t=0$ to be $\rho_0$. 

Assume $P(t) = P e^{\omega t}, C(t) = C e^{\omega t}$. Substitute into PKEs, we get 
\eqn{ \omega P &= \frac{\rho_0 - \beta}{\Lambda} P + \lambda C & \omega C &= \frac{\beta}{\Lambda} P - \lambda C} 

This is a homogeneous linear system that can be written in the matrix form: 
\eqn{ \left[ \begin{array}{cc} \omega - \frac{\rho_0 - \beta}{\Lambda} & - \lambda \\ - \frac{\beta}{\Lambda} & \omega + \lambda \end{array} \right] \left[ \begin{array}{c} P \\ C \end{array} \right] &= \left[ \begin{array}{c} 0 \\ 0 \end{array} \right] }

Non-trival solution exists if and only if the determinant of the matrix is 0, that is, 
\eqn{ \Lambda \omega^2 + (\lambda \Lambda + \beta - \rho_0) \omega - \rho_0 \Lambda &= 0 }
whose solution is, 
\eqn{ \omega = \frac{1}{2\Lambda} \left[ - (\beta - \rho_0 + \lambda \Lambda) \pm \sqrt{ (\beta - \rho_0 + \lambda \Lambda)^2 + 4 \lambda \Lambda \rho_0 } \right] }
That is, we arrive at $P(t) = P_1 e^{\omega_1 t} + P_2 e^{\omega_2 t}, C(t) = C_1 e^{\omega_1 t} + C_2 e^{\omega_2 t}$. Assume $\lambda \Lambda \ll \beta, |\rho_0| \ll \beta$, we can approximate 
\eqn{ \omega_1 &\approx \frac{\lambda \rho_0}{\beta - \rho_0}, & \omega_2 &\approx - \frac{\beta - \rho_0}{\Lambda} }
 Then 
\eqn{ P(t) \approx P_0 \left[ \frac{\beta}{\beta - \rho_0} e^{\frac{\lambda \rho_0}{\beta - \rho_0} t} - \frac{\rho_0}{\beta - \rho_0} e^{ \frac{\rho_0 - \beta}{\Lambda} t} \right] }

\begin{figure}[h]
  \centering
  \includegraphics[width=6in]{images/pke/PKE-ana-sol.png}
  \caption{Plot of Analytical Solution of PKE} \label{PKE-ana-sol}
\end{figure}

\clearpage
\topic{Numerical Solutions of Various Senarios}
\begin{enumerate}
...
Prompt-jump approximation is used to estimate prompt-jump (of course). Stacy states that `if we are not interested in the details of the prompt neutron kinetics during the prompt jump, we can simplify the equations by assuming that the prompt jump takes place instantaneously in response to any reactivity change, and afterward, the neutron population changes instantaneously in response to changes in the delayed neutron source.'
\begin{enumerate}
...
\topic{Inhour Equation / Nordheim Equation}
Prof. Forget defines reactor period as `the power level divided by the rate of change of power': 
\eqn{ \tau(t) &= \frac{ \frac{p(t)}{\derivative p(t)}   }{\dt} }
Implications: 
\begin{enumerate}
\item $\tau = \infty$ means steady-state. 
\item Small positive $\tau$ means a rapid increase in power. Small negative $\tau$ means a rapid decrease in power. 
\item If period is constant, power varies exponentially: 
  \eqn{ P(t) = P_0 e^{t/\tau}} 
\item For one delayed precursor gropu, the reactor period can be separated into two parts (prompt period and stable period) which are the two exponential of the solution with different coefficient. Recall Fig.~\ref{PKE-ana-sol}. 
\item After the initial transient, there is a fixed relation between the stable period and reactivity: 
\eqn{ \tau_{\mathrm{stable}} = \left\{ \begin{array}{cc} 
\frac{\beta - \rho}{\lambda \rho}  & \rho < \beta \\
\frac{\Lambda}{\rho - \beta} & \rho > \beta 
\end{array} \right. }
\item Typical values: when $\rho < $1 mk, the period is larger than 1 min; when $\rho \approx \beta = $ 6 mk, $\tau \sim 1 \fsp \s$. 
\end{enumerate}
...
Implications:
\begin{itemize}
\item \textbf{Asymptotic periods $\tau_0 = \frac{1}{\omega}$}: we can only use Inhour equation for asymptotic period, that is, a stable condition indicated by a constant period. Thus to use Inhour equation, we have to make the reactor stable first. For any time dependent problem, we have to use the more general IK. 
\item We can compute $\rho$ using inhour equation if we know $\beta_i, \omega, \lambda_i$.
\item A typically assumption is to set $\omega \Lambda \to 0$ because the prompt neutron lifetime is so small (with the exception of super prompt in which case $\omega$ is huge). Another way to put it is that the frequency of the reactor is going to be small. 
\end{itemize}
...
\begin{enumerate}
\item Make a perturbation (e.g., withdraw control rod) on a critical reactor.
\item Measure asymptotic reactivity period $T_0$: make sure $T$ is constant before we measure it!
\item Infer the asymptotic frequency $\omega = \frac{1}{T_0}$. 
\item Use inhour equation to compute reactivity: plug $\omega$ into inhour equation. 
\end{enumerate}
There are a couple of potential issues,
\begin{enumerate}
\item The requirement of waiting for asymptotic period puts a restriction on the reactivity: if the reactivity is too large or too small then we might go over the range in side of which we can ignore feedback. 
...
Inhour equation can be used to calibrate control rods and to measure small sample worths are measured. For instance, to measure HZP rod worth due to Boron dilution, the steps are: 
\begin{enumerate}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Inverse Kinetics %%%%%%%%%%%%%%%%%%%%%%%
\clearpage
\topic{Derivation of IK} \label{derivation-of-IK}
...
The red lines in Fig.~\ref{ik-ex1} point out the location of the excore detectors. These ex-core detectors are used to monitor core flux/power level and axial power shape. 
\begin{figure}[ht]
  \centering
...
Prompt neutron life time is so short that spatial distribution of flux changes rapidly, thus we can assume that it stabalizes already at each rod drop. 
\begin{itemize}
...
In this example, we would scram the core, then measure power change and infer $T(t)$, thus estimating the reactivity. 
\begin{itemize}
\item The rough method uses Prompt Jumpt Approximation. 
\item More accurately, we need to model all the neutron sources, including ($\alpha,n$) reactions etc. 
\item In reality, there are noisy signals in measured power and reactivity data; we need to smooth them out with a fitted function before we can apply IK with external neutron sources. 
\item The point is that we can use IK instead of prompt jump approximation to gain precision. 
\end{itemize}

This technique is not used in the US that much because it is harsh on your system and it requires work to bring the system back to critical. But it is one of the best ways to demonstrate to the licensing committee that a reactor is capable of scraming.  


\subtopic{Dynamic Rod Worth} \label{dynamic-rod-worth}
