...
\item Lethargy
\item Mean free path
\item 1/v absorption cross section: Henry (p. 202) has a derivation for why $\sigma_a(E_r) \approx \frac{1}{v_r}$, but it is using Breit-Wigner for resonance xs. 
\item 1/E spectrum: true except when resonant absorber is present (recall we keep using 1/E for spectrum above the resonance). It is characteristic of the scalar flux in the slowing down energy range, as long as there is no up-scattering and no fission source. It arises basically from the kinematics of the scattering interaction. However it gets distorted by the energy behavior of the scattering cross sections and by neutron-absorption processes (Henry, p. 90). 
...
\begin{itemize}
\item thermal elastic scattering
...
\item Resonance integrals: represents the average absorption xs characterizing the resonance, averaged over the flux within the resonance. It is useful in generating multigroup xs. 
...
\item Resonance escape probability $p$:
\item Homogeneous/heterogeneous equivalence
\item Escape cross section $\sigma_e$: Smith vs. Reuss (7.2.4). 
