...
The motivation is that we want a method that lives on coarse mesh. Using CMFD on a reactor application would yield somewhere between 5-10\% error. 
...
\item 60s-70s:
  \begin{enumerate}
  \item Pin-cell cross section generation:
  \item Radial: generate F-xy peaking (axial plane peaking). 
  \item Axial: generate A-O (axiao offset). 
  \item Transient: 
  \end{enumerate}
  
\item Users want direct 3D computations, thus we do 3D steady-state multigroup neutron diffusion equation. 
  \begin{enumerate}
  \item Mesh spacing: instead of $<$ 1cm mesh that finite difference solution needs, we want mesh spacing on the order of 20 cm. 
  \item FLARE BWR Nodal model (1964): one energy group, a source in node $p$ is related to itself and its neghboring node ($q$). There is not much physics in this model, because the coefficients are derived assuming two infinite half planes. 
  \item 70s-80s advanced nodal diffusion methods have the following goals:
    \begin{itemize}
  ...
  \item Modern adnvace nodal codes: there are numerous of them, and they should all be the same. 
  \end{enumerate}
...
Reference: Sutton and Aviles' `Diffusion Theory Methods for Spatial Kinetics Calculations' (1996). \hi{Nodal methods are just a way of providing the higher-order coefficients that describe the net current in terms of node-averaged fluxes}: 
...
\clearpage
\topic{Transverse Integration}
We need a set of equations for the surface average currents instead of solving 3D finite different equations directly. To do so, we pick a direction of interest (eg., x direction), and perform integration within node over 2D plane normal to that direction (eg., the grey surface in Figure~\ref{nodal-current}), then devide by the planar area. The LHS of Eq.~\ref{nodal-balance} becomes, 
...
The RHS becomes not interesting after a while because it is just the average flux times cross sections. 
 \begin{enumerate}
  ...
  \item Interpretations: 
    ...
    \item Why nodal methods work? As the node becomes bigger and bigger, it is less likely that a random neutron to stream right through the node (thus contributing to the $\vecJ$). That is, \hi{$\vecJ$ streaming out the node from the right side is insensitive to the transverse leakage term from the top and the bottom.} Thus the transverse leakage terms $L_y(\xi_x), L_z(\xi_x) \cdots$ can be accurately represented by 2nd order polynomials. [FIXME] insert image. 
    \end{itemize}
  \end{enumerate}

\textbf{Approximation On Transverse Leakage: Quadratic Approximation in Each Node} (Finnemann, KWU). 
...
\item Semi-Analytic Nodal Method (SANM) developed at Studsvik (1985). 
  Motivation: NEM does a good job in the fast group but not that well in the thermal group, so we use the analytical solution for the thermal group. 
...
\item Summary. SANM is advantage for two reasons: [FIXME]
  \begin{enumerate}
