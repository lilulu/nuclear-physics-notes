...
\item PKE expression: 
    ...
    Physical terms: 
      \begin{enumerate}
      \item Reactivity $\rho$, where the top is the diffusion equation, and the bottom is fission rate if all the neutrons show up instatenously. $\displaystyle  \rho(t) = \frac{\keff - 1.0}{\keff}$. 
        \scriptsize
        \begin{align}
...
\boxed{ \approx \frac{1}{v \nu \Sigma_f} \xrightarrow{\mathrm{criticality}} \frac{1}{v \Sigma_a} }   \notag }
    ...
    \item Prompt-jump approximation: assume $C_i^+ = C_i^-$, $\left. \dTdt \right|_{t = 0^-} = 0, \left. \dTdt \right|_{t = 0^+} = 0$, 
      \begin{align}
...
\item Inverse Kinetics Equation: 1st order ODE again. First given $T(t)$ we solve for the precursor concentration,
