...
Then we can get the net current in terms of mesh fluxes (\textcolor{red}{notice the $J_n^+, J_n^-$ here are net currents not partial currents!!!}): 
...
\begin{enumerate}
\item Zero Flux BC: flux in node $N$ going from $\phi_g^N$ at the center to 0 at the BC. We can either think of the flux as keep going to be $-\phi_g^N$ in the imaginary-right-node, or calculate the slope by doing $\phi_g^N / (\Delta /2)$,
...
\begin{align}
  ...
  &= \boxed{ \frac{2D_g^N}{\Delta^N} \left[ \frac{1}{1 + \frac{4 D_g^{N+1}}{\Delta^N}} \right] \phi_g^N } \label{JgN}
\end{align}
In Eq.~\ref{JgN}, notice $D_g^{N+1}$ does not really exist, but we make it up so that we can write multiple BCs in the same form. That is, if $D \to 0$, we get zero flux boundary condition; if $D \to \infty$, we get zero current boundary condition. So we can actually use Eq.~\ref{JgN} for three different conditions: zero current, zero flux, zero incoming current. 
\end{enumerate}
...
\subtopic{Method 3: Power Iterations with Matlab's Direct Matrix Inverter}
...
\subtopic{Method 3+: Power Iteration with Matlab's GMRES Numerical Invertion} 
With no pre-conditioner, GMRES is actually longer than Method 3. 


\subtopic{Method 4: Power Iteration with Point-Jacobi Iterative Flux Matrix Inversion}
In a steady-state problem, it does not matter whether we fully converge our flux iteration. $m$ is fission iteration, $i$ is .
\begin{figure}[ht]
  \centering
...
\end{figure}
The outter loop is on $i$ which is the fission source iteration index, 
\begin{align}
A \phi^{i+1} &= \frac{1}{\keff} M \phi^i \\
\keff^{i+1} &= \frac{M \phi^{i+1}}{M \phi^i} \\
A &= D + O \Rightarrow D \phi^{m+1} = \frac{1}{\keff} M \phi^i - O \phi^m 
\end{align}
The inner loop is on $m$ which is the flux iteration index, 
\eqn{ \phi^{m+1} = D^{-1} \left( \frac{1}{\keff} M \phi^i - O \phi^m \right) }




\subtopic{Method 5: Power Iteration with Gauss-Seidal Iterative Flux Matrix Inversion} 
The outter loop is almost the same as Point-Jacobi, except the A is handled differently, 
\begin{align}
A &= L + D + U  \\
(L + D ) \phi^{m+1} &= \frac{1}{\keff} M \phi^i - U \phi^m 
\end{align}
Then the inner loop is on $m$ again which is the flux iteration index, 
\eqn{ \phi^{m+1} &= (L + D)^{-1} \left( \frac{1}{\keff} M \phi^i - U \phi^m \right) }
\textcolor{blue}{Notice these Point-Jacobi and Gauss-Seidal only work with 2D problem that we can easily split a matrix into L, D, U, and they would not work well with 3D and above.} 
