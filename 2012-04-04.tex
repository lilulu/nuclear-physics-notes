...
\end{align}
In Eq.~\ref{JgN}, if $D \to 0$, we get zero flux boundary condition; if $D \to \infty$, we get zero current boundary condition. So we can actually use Eq.~\ref{JgN} for three different conditions: zero current, zero flux, zero incoming current. 
\end{enumerate}
