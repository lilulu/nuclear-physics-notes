...
\item Slowing down region/resonance region $[1\eV, 0.1 \MeV]$. We define a slowing down density: 
  ...
  \begin{align}
    q(E) &= - \int_E^{\infty} \Sigma_a (E') \phi(E') \dE' + S_f \label{eqn:qE}
  \end{align}

  Next we apply the second fundamental theorem of calculus, or the
  Newton-Leibniz axiom, which states that, 

  \eqn{ F'(x) &= f(x)  &\Rightarrow F(b) - F(a) &= \int_a^b f(x) \dx}

  Be careful about the bounds as the second theorem is not simply
  taking the derivative of Eq.~\ref{eqn:qE}. In our case, $q = F,
  \Sigma \phi = f$. We already have $q(E)$, substitute $E = \infty$ in
  Eq.~\ref{eqn:qE} we get,

  \eqn{ q(\infty) &= S_f \label{eqn:qE-infty}}

  Then Eq.~\ref{eqn:qE-infty} minus Eq.~\ref{eqn:qE} yields, 

  \eqn{ q(\infty) - q(E) = \int_E^{\infty} \Sigma_a (E') \phi(E') \dE' }

  which is the second expression in the second fundamental theorem of
  calculus. Then by the theorem,
  ...
  \begin{itemize}
    ...
    \item $q(E)$ changes proportional to $\Sigma_a$, because in our approximation the source term becomes indpendent of energy. 
