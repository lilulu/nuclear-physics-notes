...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\clearpage
\topic{Sample Diffusion Problems From Lectures}
\begin{enumerate}
...
Answer: asymptotically, any norm (L1, L2, L$\infty$) can be used to compute convergence or dominance ratio. If using L$\infty$, eps1 = $\frac{1.99-1.98}{1.99} = 0.005025$, eps2 = $\frac{2.00-1.99}{1.99} = 0.005$, then the DR = $\frac{eps2}{eps1} \approx 0.995$. 
...
\item (12 Exam 2 \#10) What is the energy-independent critical material buckling and fast-to-thermal flux ratio for Fuel 1 material? \\
\textbf{Answer:} write out the two group diffusion equations with $D_1 B^2, D_2 B^2$ and solve for $B^2$. Notice the hint of energy-independent critical material buckling, implying that the two groups share the same $B^2$. 

\item (12 Exam 2 \#11) Los Alamos scientists have created a perfect neutron reflecting material, Reflectium. If a homogeneous hemisphere is placed flat side down on an infinitely large table of Reflectium, what is the critical radius of the hemisphere? \\
\textbf{Answer:} Don't forget extrapolation distance!
\eqn{ B^2 &= \left( \frac{\pi}{\tilde{R}} \right)^2 & \tilde{R} &= R + d }

\item (12 Exam 2 \#12) I.M. Lazy wants to analyze a finite slab reactor of Fuel 1 with infinitely thick pure water reflector on both sides. I.M. decides to perform a two-group zero-leakage infinite-medium spectrum calculation for Fuel-1 material and collapse one-group cross sections and diffusion coefficients for both Fuel-1 and Reflector material using that spectrum. What critical thickness will he compute with his collapsed 1G cross section data? \\
\textbf{Answer:}

\item (12 Exam 2 \#13) FP
\textbf{Answer:}

\item (12 Exam 2 \#14) FP
\textbf{Answer:}
\end{enumerate}
