...
\subtopic{Adding Chemical Bound: Bound Elastic Scattering of H in Water Molecule}
...
Apply assumptions:
\begin{enumerate}
\item Assume steady state, so $\frac{1}{v} \pphipt \to 0$. 
\item Assume infinite medium, so no streaming term $\Omega \gradient \phi \to 0$. 
\item Assume no source, so $S \to 0$. 
\end{enumerate}
If we have the time dependency part, flux changes with respect to time and is driven by source. Once we go into steady state and take away the source, \textbf{TE becomes an eigenvalue problem, and we need to add a $k$ on the bottom of the fission term}, also notice that our TE only depends on energy now,  
...
\item Slowing down region/resonance region $[1\eV, 0.1 \MeV]$. We define a slowing down density: 
  ...
  Again assume no upscattering, 
  \eqn{ q(E) = \int_E^{\infty} \chi(E') S_f \dE' - \int_E^{\infty} \Sigma_a(E') \phi(E') \dE' }
  In the presence of scattering, all neutrons born from fission will reach $E$ unless absorbed. That is, from the general expression $\int_0^{\infty} \chi(E') \dE' = 1$, we can approximate $\int_{0.1}^{\infty} \chi(E') \dE' = 1$, thus only leaving the energy independent $S_f$ term:  
  \begin{align}
    q(E) &= - \int_E^{\infty} \Sigma_a (E') \phi(E') \dE' + S_f \\
    &= \int_{\infty}^E \Sigma_a (E') \phi(E') \dE' + S_f  \\
  ...
  \begin{itemize}
    ...
    \item $q(E)$ decreases proportional to $\Sigma_a$. 
...
In reactor applications, most absorptions happen in the resonances of \ce{^{238} U}. \ce{^{238}U}'s cross section changes by order of magnitudes for each resonance, so we can assume that $\Sigma_a = 0$ between resonances (also because between resonances there is no quantum effect between compound nuclides). That is, $q(E)=$ constant between resonances. Consider our TE again with $\Sigma_a = 0$, 
      ...
      Motivated by the above image, we define $q(E)$ in terms of scattering, 
      ...
      \eqn{ \Aboxed{ \phi(E) &= \frac{q(E)}{\xi \Sigma_s (E) E} }  }
      In lethargy space, 
      \eqn{ \Aboxed{ \phi(u) &= \frac{q(u)}{\xi \Sigma_s (\mu) } } }
      ...
      \eqn{ \Aboxed{ \phi(E) &\approx \frac{1}{E} } } 
      \end{itemize}
