...
\uline{Potential 1: Infinite Wall} 

It does not work because it requires infinite amount of energy to separate a neutron or a proton; model has sharp edge, whereas nuclear charge and matter distribution fall smoothly to zero beyond the mean radius. 

\uline{Potential 2: Harmonic Oscillator}

It does not work because the energy goes to infinity, and the potential inside of the well is not sharp enough (compare with experimental results);
\begin{enumerate}
...
\item Modified Harmonic Oscillator: know spin-orbit coupling: $V_{\mathrm{nuc}} = V_0 + V_{\mathrm{so}} \frac{\hat{l} \cdot \hat{s} }{\hbar^2}$. 
  \begin{align}
...
\textbf{10 Quiz 2 \#3b}: The measured assignment for \ce{^{61}_{28}Ni} is $\frac{3}{2}^-$. Explain why this differs from predicted in 3a. 
